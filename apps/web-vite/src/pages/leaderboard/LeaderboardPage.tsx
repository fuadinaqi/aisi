...
export default function LeaderboardPage() {
  const { data, isLoading } = useQuery<
    { id: string; name: string; totalPoints: number; roles: { role: string }[] }[]
  >({
    queryKey: ['leaderboard'],
    queryFn: async () =>
      (await api.get<
        ApiResponse<{ id: string; name: string; totalPoints: number; roles: { role: string }[] }[]>
      >('/points/leaderboard?limit=50')).data.data,
  });
