...
import { emptyValueForItem } from '@/lib/mutabaah';
...
export default function MutabaahPage() {
  ...
  useEffect(() => {
    ...
    for (const item of data.items) {
      next[item.id] = item.value ?? emptyValueForItem(item);
    }
    setAnswers(next);
  }, [data?.items, data?.id]);
  ...
  const buildPayload = () => ({
    ...
    answers: items.map((item) => ({
      itemId: item.id,
      value: answers[item.id] ?? item.value ?? emptyValueForItem(item),
    })),
  });
  ...
  const save = async (submit = false) => {
    try {
      ...
      if (submit) {
        ...
        toastSuccess('Mutabaah berhasil dikirim');
      } else {
