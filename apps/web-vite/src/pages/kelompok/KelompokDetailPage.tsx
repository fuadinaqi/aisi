...
export default function KelompokDetailPage() {
  ...
  const school = group.school ?? { id: '', name: '-' };
  const pembina = group.pembina ?? { id: '', name: '-', email: undefined, phone: null as string | null };
  ...
  const backHref =
    role === 'PEMBINA' ? '/dashboard' : `/schools/${school.id || '/schools'}`;
  ...
  return (
    <PageContainer tight className={cn('relative', theme.pageGlow)}>
        ...
        action={
          canManageMembers ? (
            <div className="flex items-center gap-2">
              ...
              >
                <Link to={`/kelompok/${id}/anggota/undang`}>
      ...
      <p className="px-0.5 text-sm text-muted-foreground">{school.name}</p>
        ...
        <ListGroup className={cardClass}>
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <p className="mt-1 font-medium">{pembina.name}</p>
              {pembina.email && (
                <p className="text-sm text-muted-foreground">{pembina.email}</p>
              )}
            </div>
            <WhatsAppButton phone={pembina.phone} size="sm" />
          </div>
        </ListGroup>
            ...
            group.members.map((m, i) => (
              <div key={m.user.id}>
                ...
                <div className="flex items-start gap-3 px-4 py-4 md:px-5">
                  <Link
                    to={`/kelompok/${id}/anggota/${m.user.id}`}
