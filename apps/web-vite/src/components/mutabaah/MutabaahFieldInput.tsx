...
function asSelectValue(value: MutabaahAnswerValue | null | undefined): MutabaahSelectAnswer {
...
export function MutabaahFieldInput({
  item,
  value,
  disabled,
  onChange,
  ...
  const safeValue = value ?? emptyValueForItem(item);
  if (item.inputScope === 'DAILY') {
    const days = getDailyDays(item, safeValue);
  ...
  switch (item.fieldType) {
      ...
      return (
        <label className="flex cursor-pointer items-center gap-3 rounded-xl border px-4 py-3">
          <input
            type="checkbox"
            disabled={disabled}
            checked={Boolean((safeValue as { checked?: boolean }).checked)}
    ...
    case 'NUMBER':
      return (
        <Input
          ...
          disabled={disabled}
          placeholder="0"
          value={formatNumberInputValue((safeValue as { value?: number }).value)}
    ...
    case 'TEXT':
      return (
        <textarea
          disabled={disabled}
          rows={3}
          value={String((safeValue as { text?: string }).text ?? '')}
    ...
    case 'SELECT': {
      ...
      const selectValue = asSelectValue(safeValue);
