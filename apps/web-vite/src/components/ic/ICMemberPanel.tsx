...
export function ICMemberPanel({ userId, groupId, canEdit = false }: Props) {
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toastSuccess('Progress IC diperbarui');
    },
    onError: (err: unknown) => {
      toastError(err, 'Gagal memperbarui progress IC');
    },
  });
  ...
  if (!data?.summary || !Array.isArray(data.categories)) {
    return (
      <div className="rounded-xl border border-dashed border-border/60 px-4 py-6 text-center">
        <p className="text-sm text-muted-foreground">Belum ada data indikator capaian</p>
      </div>
    );
  }

  const { summary, categories } = data;
  const percent = icProgressPercent(summary.achieved ?? 0, summary.total ?? 0);
