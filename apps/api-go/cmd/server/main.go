...
func main() {
	...
	pool, err := db.Connect(ctx,cfg.DatabaseURL); if err != nil { log.Fatal(err) }; defer pool.Close()
	server:=&http.Server{Addr:":"+cfg.Port,Handler:httpx.Router(pool,cfg),ReadHeaderTimeout:10*time.Second}
	go func(){log.Printf("AISI Go API mendengarkan pada :%s",cfg.Port);if err:=server.ListenAndServe();err!=nil&&err!=http.ErrServerClosed{log.Fatal(err)}}()
